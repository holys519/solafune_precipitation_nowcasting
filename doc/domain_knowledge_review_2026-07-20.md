...
## 3. 未実施・green・実装コスト低い候補 (優先順位付き)

| 優先度 | 候補 | 根拠 | 実装コスト | 備考 |
| --- | --- | --- | --- | --- |
| **1** | local solar hour の sin/cos | `imerg_physics_notes.md` E-9で**既に検証済み**: GOES圏振幅0.136(16時ピーク)、Himawari圏4時ピーク、Meteosat圏0.042(16時)。ノイズ超え確定、閉形式で許可済み | 低(緯度経度+時刻から計算) | **最有力。検証済みで未実装なのはこれだけ** |
| 2 | mir(3.9μm)-win(10.5μm) BTD (霧/下層雲判別) | 文献確立手法。今回のclear-sky合成でbihar/dhakaの"曇天なのに無降水"を説明する仮説と直結 | 低(既存帯を差分するだけ) | 昼夜で挙動が変わる点に注意(文献: 昼は正で角度依存、夜は負で一定) |
| 3 | IR window局所分散 (テクスチャ、対流/霧判別) | 文献確立手法、overshooting top検出に有効 | 中(3×3などの局所窓フィルタを追加) | 空間フィルタなので受容野・パディングの実装確認が必要 |
| 4 | GPI風 rain-area-fraction (win<235K相当の面積比) | 古典的でシンプル、教師なしオラクル/補助特徴の両方に使える | 低 | uint8生値なので235K換算に一次校正が必要(データから経験的に決める) |
| 5 | split-window BTD (差分版、比ではなく) | 文献の標準形。既存の比バージョンとのアブレーション | 低 | 量子化耐性は比より劣るはずだが試す価値あり |
