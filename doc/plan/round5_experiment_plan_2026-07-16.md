...
**g_eda/exp003 完了** (`outputs/g_eda/exp003/BLEND_CURVE.md`):

| 構成 | OOF tile_rmse |
| --- | ---: |
| equal(016,017) | 0.6062 |
| ladder w018=0.50 (≒提出済みexp033) | 0.60032 |
| ladder最適 w018=0.45 | 0.60024 (差8e-5 — **w=0.5はほぼ最適、`w018_075`は+0.0027悪化なので提出不要**) |
| 3-way global最適 (0.25/0.30/0.45) | 0.60020 (差4e-5 — 単独提出の価値なし) |
| 衛星別最適トリプル合成 | 0.59906 (goes: 018を15%だけ / him: 55% / met: 60%) |
| **合成: 衛星別 + blur σ1.0 + threshold 0.2** | **0.59713 (ladder w=0.5比 −0.0032)** |

- 単軸はすべてノイズ閾値(0.004)未満だが、合成は−0.0032でw018_050の実測LB改善(−0.0027)と
  同規模の期待値 → **`exp036 per_satellite + blur1.0 + thr0.2` を1枠で提出する価値あり**
  (zip生成ジョブ投入済み)。
- 注意: OOFキャッシュは素のモデル出力 (TTA・per-model threshold 0.10なし) で、提出パイプ
  ラインと完全一致ではない。threshold分の一部は既に提出物に織込み済みのため、実効LB改善は
  −0.002前後と見るのが保守的。
- GOESだけexp018の重みが小さい (15%) — exp018のGOES localization劣化 (E-1: goes actual
  0.8530 > exp016/017) と整合。exp035系の次期モデルはGOESの回帰を採用条件でチェックする。

**exp035 fullアーム A/B**: fold0 0.29485 / fold4 0.58837 — **両foldでexp018に敗北、ゲート
不通過**。exp028/exp030のfold0勝ちはexp018の高解像アーキ上で再現せず (E-3の教訓の実例)。
分離アーム no_dilation / dilation_only / tilemean (exp018素+tile-mean lossのみに変更済み)
のfold0+4を投入済み (jobs 3935475-3935480)。

**exp031 (focal+tversky, fp32修正後)**: fold0 0.30835 = exp017比+0.0019 → **クローズ**
(計画通り: フラットなら閉じる)。

## 11. exp036実測 0.6706858 (LB 29位) と「未発見の工夫」の棚卸し (2026-07-16 追記)

`exp036_per_satellite_blur1_thr0p2_patched` = **0.6706858062196032** (exp033比 −0.0013、
OOF予測−0.0032に対し実測は約4割 — threshold分が既存パイプラインの per-model thr 0.10 と
重複していたためで、保守見積り通り)。LBスナップショット更新: 1位 Bull 0.63474、
トップとの差 0.036、トップ10と 0.022。

### 未回収だった軸の棚卸しと着手状況

| 軸 | 根拠 | 状態 |
| --- | --- | --- |
| **時間平滑化** | GPM lag1(30分)自己相関0.561。inference.pyに実装済みなのに全configで `enabled: false`、一度も検証されていなかった | **g_eda/exp004でOOF実証: −0.0035** (center0.25/prev·next0.375)。片側probeで **next(未来)フレームがprevの3倍効く** (t+0情報ピークと整合) → 非対称グリッド実行中 |
| **rot90 TTA** | 学習augはflip+rot90だがTTAはflip 3-viewのみ。公開0.69解法は6-way、G-034で計画のまま未実施 | **exp037実装+投入済み** (job 3935541): 3モデルを8-viewで再推論→combo→patch。対照はexp036そのものなのでLB A/Bが成立 |
| rain_prob_threshold | exp034ビルド済み・未提出 | 提出枠次第 |
| GPM 0.01量子化スナップ | exp038計画のまま | OOFキャッシュで即検証可能 (次) |
| 衛星別value_threshold / 衛星別isotonic | exp036は全衛星一律thr 0.2 | OOFキャッシュで即検証可能 (次) |
| exp018マルチシード | exp025はexp017のみ。exp027の教訓 (OOF重み+弱メンバー除外) 適用が条件 | GPU重、exp035分離アーム結果後 |
| median/IQR正規化・50binカテゴリカルhead | ディスカッション §2-3、未実装 | 学習系、Round 6候補 |

### 運用変更

- 以後のブレンド系提出は、exp037完了後は**TTA済み予測をsourceに恒久切替**。
- 次の提出候補 (優先順): ①exp036 + `--smooth` (OOF確定値) ②exp037 (TTA A/B)
  ③exp034 thr_w018_000。いずれも既存zipまたはジョブ完了待ち。

## 12. 平滑化の実測 0.66617 (−0.0045) と次アプローチ (2026-07-16 追記)

`exp036_per_satellite_sm0p25_blur1_thr0p2_patched` = **0.6661746681900441** (LB ~24位相当)。
実測−0.0045 > OOF予測−0.0038 — eval側の時系列は隣接可用性99.5%でtrainよりむしろ濃く、
時間平滑化はこのパイプライン最大の単発改善になった。

### 直後に実施した再最適化 (g_eda/exp004 stage 2, `JOINT_POSTPROCESS.md`)

| 変更 | OOF効果 |
| --- | --- |
| 平滑化を5タップ (±60分) へ拡張 (0.18/0.22/0.32/0.11/0.17) | −0.0011 |
| 衛星別平滑化重み | −0.0001 (微小) |
| blur再調整: σ1.0→**0.5** (平滑化が平均化を肩代わり) + 衛星別threshold (goes 0.15 / him 0.30 / met 0.25) | −0.0008 |
| GPM 0.01量子化スナップ | **±0 → G-025クローズ** |
| **合計 (現servingスタック比)** | **−0.0019** |

### 提出キュー (生成済み/生成中)

1. `exp037_per_satellite_sm0p25_blur1_thr0p2_patched.zip` — **rot90 TTAのA/B** (差分はTTAのみ、対照0.66617)
2. `exp036_per_satellite_joint_patched.zip` (job 3935582) — 上表の再最適化スタック (OOF −0.0019)
3. TTAが勝てば joint スタックをTTA sourceで再ビルド (`exp037 --blend-only` の config を joint に差替え)

### TTA A/Bの結果 (同日追記)

`exp037` = 0.666259584999578 vs 対照 0.6661746681900441 → **+0.00008の完全なタイ。
rot90 TTAは効果なしと判定、exp037クローズ**。source切替はせず、以後も非TTA予測を使う
...
- **exp035 no_dilation が中間経過で有望**: fold0 途中best 0.28694 (exp018 0.29234比 −0.0054、
  fullアーム0.29485比 −0.0079)。**fullを壊していたのはdilated bottleneck**とほぼ確定。
  完走後、fold0+4両方で勝てば5-fold本走 → 第4ブレンドメンバー → g_eda/exp003/004を
  4-source で再実行(ブレンド重み+後処理の再収穫)。
- **Round 6 学習テーマの筆頭 = t+0中心の時間設計 (exp019/G-033)**: 平滑化の片側probeで
  next:prev = 3:1 が実測された今、根拠が最も強い。予測レベルの平滑化で−0.0045取れる
  ということは、モデルが時間文脈を使い切れていない証拠 — 入力レベルで隣接行を明示的に
  扱う設計 (t+0 primary + |Δ|) の期待値が上がった。
- exp018マルチシード (OOF重み+弱メンバー除外) はその後。
