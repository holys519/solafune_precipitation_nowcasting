...
| exp064_convnext_lr2e4 | green、**LB確認済み 0.6837550386507026 (2026/08/02) — exp064系最下位** | context_rows: 1。timm `convnext_tiny` pretrained encoder、lr=2e-4。5-fold OOF 0.60442はeffv2s相当で期待したが実測はconvnext_small比+0.00402悪化。ConvNeXt系統はサイズ非依存で弱いと確定、軸クローズ。ImageNet重みMIT |
| exp064_effb1 | green、**未提出(zip構築済み、LB未確認)** | context_rows: 1。timm `efficientnet_b1` pretrained encoder、effb3から一段小さいサイズ。fold0/4ゲートはeffb3自身比クリーンpass(f0-0.00572/f4タイ)。5-fold OOF 0.59934。ImageNet重みApache-2.0 |
| exp064_effb2 | green、**未提出(zip構築済み、LB未確認)** | context_rows: 1。timm `efficientnet_b2` pretrained encoder。fold0/4ゲートはeffb3自身比クリーンpass(f0-0.00504/f4タイ)。5-fold OOF **0.59693 — effb3自身のOOF(0.59758)を上回る**、今セッション単体2位。「EfficientNetは小さい方が勝つ」パターンがB3からさらに一段下でも継続。ImageNet重みApache-2.0 |
| exp064_regnety016 | green、**未提出(zip構築済み、LB未確認)** | context_rows: 1。timm `regnety_016` pretrained encoder、新backbone系統(RegNet-Y)。fold0/4ゲートはmixed(f0全候補中2位の-0.00808、f4+0.00632悪化)。5-fold OOF 0.60025。ImageNet重みApache-2.0 |
| exp064_densenet121 | green、**未提出(zip構築済み、LB未確認)** | context_rows: 1。timm `densenet121` pretrained encoder、新backbone系統(DenseNet)。fold0/4ゲートはmixed(f0全候補中最良の-0.01007、f4+0.00846悪化)。5-fold OOF 0.59990。ImageNet重みApache-2.0 |
...
| exp065_champion_ensemble_11way_causal | green、**未提出(zip構築済み、LB未確認) — submission_gate.py GO判定 (point delta -0.01417、80%CI `[-0.01726,-0.01130]`)** | 6-wayにconvnext_lr2e4/effb1/effb2/regnety016/densenet121を追加した11-wayアンサンブル。honest nested score 0.58341(6-way比-0.00152改善)、後処理込み最終OOF 0.58279(6-way比-0.00139改善)。outer-cross-fit重みでconvnext_lr2e4/effv2s/regnety016は実質0に自動収束、effb1(0.1465)/effb2(0.1163)/densenet121(0.1454)が真の貢献者。次の提出枠での最優先候補 |
