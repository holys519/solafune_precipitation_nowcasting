...
#SBATCH --time=60
#SBATCH --output=slurm-g-eda-exp003-4src-%j.out
#SBATCH --error=slurm-g-eda-exp003-4src-%j.err
...
if [ -n "${SLURM_SUBMIT_DIR:-}" ] && [ -f "$SLURM_SUBMIT_DIR/run_4source_blend.py" ]; then
...
if [ ! -f "$PROJECT_DIR/outputs/g_eda/exp003/exp035_no_dilation_oof_pred.npz" ]; then
  run_py "$SCRIPT_DIR/run_blend_curve.py" --cache exp035_no_dilation \
    --module-dir exp035 --checkpoint-dir exp035_no_dilation
fi
run_py "$SCRIPT_DIR/run_4source_blend.py"
