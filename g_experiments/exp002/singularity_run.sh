...
#SBATCH --time=720
#SBATCH --output=slurm-exp002-%j.out
#SBATCH --error=slurm-exp002-%j.err
...
if [ "$CONFIG" = "all" ] || [ "$CONFIG" = "all_submit" ] || [ "$CONFIG" = "submit" ] || \
   [ "$CONFIG" = "submission" ] || [ "$CONFIG" = "infer" ] || [ "$CONFIG" = "inference" ] || \
   [[ "$CONFIG" =~ ^[0-9]+$ ]]; then
  FOLD="$CONFIG"
  CONFIG="config.yaml"
fi
