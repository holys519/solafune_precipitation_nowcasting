#!/usr/bin/env python3
"""Create fold logs and OOF diagnostics for exp003."""
...
@torch.no_grad()
def predict_with_tta(model: nn.Module, x: torch.Tensor, amp: bool, use_flip_tta: bool) -> torch.Tensor:
...
def analyze_oof(config: dict[str, Any], checkpoint_paths: list[Path], analysis_dir: Path) -> dict[str, Any]:
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        for batch in loader:
            ...
            pred = predict_with_tta(model, x, amp=amp, use_flip_tta=use_flip_tta).clamp_min(clip_min)
