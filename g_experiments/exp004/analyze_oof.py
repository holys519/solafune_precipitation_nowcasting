#!/usr/bin/env python3
"""Create fold logs and OOF diagnostics for exp004."""
...
def detection_row(threshold: float, stats: dict[str, float], sse: float, pixels: float) -> dict[str, float]:
    tp = stats["tp"]
    fp = stats["fp"]
    fn = stats["fn"]
    tn = stats["tn"]
...
def summarize_training(config: dict[str, Any], analysis_dir: Path) -> dict[str, Any]:
    ...
    for path in metric_paths:
        ...
        best_record = min(history, key=lambda row: row["rmse"])
        fold_rows.append(
            {
                "fold": fold,
    ...
    best_values = [float(row["best_rmse"]) for row in fold_rows]
    return {
        "folds": len(fold_rows),
        "best_rmse_mean": float(np.mean(best_values)) if best_values else None,
        "best_rmse_std": float(np.std(best_values)) if best_values else None,
    }
...
def analyze_oof(config: dict[str, Any], checkpoint_paths: list[Path], analysis_dir: Path) -> dict[str, Any]:
    ...
    threshold_rows = [
        detection_row(threshold, threshold_stats[threshold], threshold_sse[threshold], threshold_pixels[threshold])
        for threshold in threshold_grid
    ...
    if threshold_rows:
        best_threshold_row = min(threshold_rows, key=lambda row: row["rmse"])
...
def main() -> None:
    ...
    analysis_dir = resolve_path(config["paths"].get("analysis_dir", "../../outputs/analysis/exp004"))
