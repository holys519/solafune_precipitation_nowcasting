"""Model architectures for exp006.
...
- "temporal_fusion_unet": shared per-observation stem before U-Net fusion.
- "satellite_adapter_unet": satellite-specific input stems before a shared U-Net.
...
class TemporalFusionUNet(nn.Module):
    """Shared per-time encoder with mask-aware fusion before a compact U-Net decoder."""
    ...
    def __init__(
        ...
        super().__init__()
        expected_channels = max_observations * satellite_channels + max_observations + 3
        if in_channels != expected_channels:
            raise ValueError(f"Expected in_channels={expected_channels}, got {in_channels}")
        self.max_observations = max_observations
        self.satellite_channels = satellite_channels
        c = base_channels
        self.time_stem = ConvBlock(satellite_channels + 1, c)
        self.satellite_stem = nn.Conv2d(3, c, kernel_size=1)
    ...
    def _split_input(self, x: torch.Tensor) -> tuple[list[torch.Tensor], list[torch.Tensor], torch.Tensor]:
        maps: list[torch.Tensor] = []
        masks: list[torch.Tensor] = []
        offset = 0
        for _ in range(self.max_observations):
            maps.append(x[:, offset : offset + self.satellite_channels])
            offset += self.satellite_channels
        for _ in range(self.max_observations):
            masks.append(x[:, offset : offset + 1])
            offset += 1
        satellite_maps = x[:, offset : offset + 3]
        return maps, masks, satellite_maps

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        maps, masks, satellite_maps = self._split_input(x)
        features = []
        for obs, mask in zip(maps, masks):
            feat = self.time_stem(torch.cat([obs, mask], dim=1))
            features.append(feat * mask)
        stacked = torch.stack(features, dim=1)
        mask_stack = torch.stack(masks, dim=1)
        valid_count = mask_stack.sum(dim=1).clamp_min(1.0)
        e1 = stacked.sum(dim=1) / valid_count
        e1 = self.fuse_refine(e1 + self.satellite_stem(satellite_maps))
        e2 = self.enc2(F.avg_pool2d(e1, kernel_size=2, ceil_mode=True))
        e3 = self.enc3(F.avg_pool2d(e2, kernel_size=2, ceil_mode=True))
        b = self.bottleneck(e3)
        d2 = F.interpolate(b, size=e2.shape[-2:], mode="bilinear", align_corners=False)
...
class SatelliteAdapterUNet(nn.Module):
    """Satellite-specific stems selected by the one-hot satellite maps in the input."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.shape[1] < 3:
            raise ValueError("satellite_adapter_unet expects satellite one-hot maps in the last 3 channels")
        satellite_maps = x[:, -3:]
...
def build_model(config: dict) -> nn.Module:
    ...
    if architecture == "temporal_fusion_unet":
        return TemporalFusionUNet(
            in_channels=in_channels,
            base_channels=int(model_cfg["base_channels"]),
            max_observations=int(config["data"].get("max_observations", 3)),
            satellite_channels=int(config["data"].get("satellite_channels", 16)),
        )
    if architecture == "satellite_adapter_unet":
        return SatelliteAdapterUNet(in_channels=in_channels, base_channels=int(model_cfg["base_channels"]))
