#!/bin/bash
# exp008 end-to-end entrypoint (no Slurm; run directly on a GPU box).
#
# Usage:
#   bash run.sh [config] [fold]
#   bash run.sh                          # config.yaml, analyze exp004 checkpoints -> exp008 submit
...
FOLD="${2:-submit}"
SOURCE_MODEL_DIR="${SOURCE_MODEL_DIR:-$PROJECT_DIR/g_model/exp004}"
...
run_inference() {
  ...
  local calibration_path="$PROJECT_DIR/outputs/analysis/exp008/oof_calibration.json"
  shopt -s nullglob
