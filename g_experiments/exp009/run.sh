#!/bin/bash
# exp009 end-to-end entrypoint (no Slurm; run directly on a GPU box).
#
# Usage:
...
run_inference() {
  local model_dir="$PROJECT_DIR/g_model/exp009"
  local calibration_path="$PROJECT_DIR/outputs/analysis/exp009/oof_calibration.json"
  shopt -s nullglob
