"""PyTorch datasets for exp016 (successor-row inputs from exp009 + zero-obs row filter).
...
def drop_zero_observation_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for row in rows:
        ...
        if observations:
            ...
            dropped += 1
    if dropped:
        print(f"dropped {dropped} zero-observation rows from training ({len(kept)} kept)", flush=True)
    return kept
