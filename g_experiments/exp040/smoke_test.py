#!/usr/bin/env python3
"""exp040 smoke test: config/channel consistency, CPU forward/backward for every arm, and a
...
ARMS = ["config.yaml", "config_metric.yaml"]
...
def check_arm(cfg_file: str) -> dict:
    ...
    # mean-shape identity checks: mean_pixels(shape) == 1, mean_pixels(pred) == mean_intensity
    shape_mean = out["shape"].mean(dim=(1, 2, 3))
    assert torch.allclose(shape_mean, torch.ones_like(shape_mean), atol=1e-3), \
        f"{cfg_file}: mean_pixels(shape) != 1, got {shape_mean}"
    pred_mean = out["pred"].mean(dim=(1, 2, 3))
    assert torch.allclose(pred_mean, out["mean_intensity"], atol=1e-3), \
        f"{cfg_file}: mean_pixels(pred) != mean_intensity, got {pred_mean} vs {out['mean_intensity']}"
...
def main() -> None:
    for arm in ARMS:
        ...
        print(result)
    check_real_batch()
    print("exp040 smoke test PASSED")
