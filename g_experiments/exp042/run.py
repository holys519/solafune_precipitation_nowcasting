#!/usr/bin/env python3
"""exp042: 5-source OOF-weighted blend (exp016/017/018/exp035_no_dilation + exp038_features)
...
ANALYSIS_DIR = ROOT / "outputs/analysis/exp042"
...
def build(name: str, weights: dict[str, dict[str, float]], rows: list[dict[str, str]],
    ...
    raw_dir = SUBMISSIONS / f"exp042/{name}_raw"
...
def apply_overlap_patch(name: str, raw_dir: Path, filenames: list[str]) -> Path:
    patched_dir = SUBMISSIONS / f"exp042/{name}_patched"
    patched_zip = SUBMISSIONS / f"exp042_{name}_patched.zip"
    patch_config = {
        "experiment": {"name": "exp042",
...
def main() -> None:
    ...
    raw_zip = create_submission_zip(raw_dir, SUBMISSIONS / f"exp042_{name}_raw.zip", filenames)
    patched_zip = apply_overlap_patch(name, raw_dir, filenames)

    summary = {
        "experiment": "exp042", "weights": weights, "postprocess": postprocess,
