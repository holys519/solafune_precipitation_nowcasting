"""PyTorch datasets for exp045: exp017's full feature pipeline (canonical bands, engineered
...
def registration_shifts_from_config(
    config: dict, resolve_path
) -> dict[tuple[str, str], tuple[float, float]]:
    """Opt-in via config data.registration_shifts_path (g_eda/exp005's merged table); absent
    or empty means no shift, so this is a strict superset of exp038's behavior when unset."""
    raw_path = config.get("data", {}).get("registration_shifts_path")
    if not raw_path:
        return {}
    return load_registration_shifts(resolve_path(raw_path))


def load_registration_shifts(path: Path | None) -> dict[tuple[str, str], tuple[float, float]]:
    """g_eda/exp005's merged train(empirical)+eval(geometric) parallax table. Only Himawari
    has a nonzero shift (the only satellite whose geometric extrapolation was validated,
    direction cosine 0.81); GOES/Meteosat stay at (0,0) for both splits, so registering one
    split but not the other never happens."""
    if path is None or not path.is_file():
        return {}
    raw = json.loads(path.read_text(encoding="utf-8"))
    return {
        (location, satellite): (float(dy), float(dx))
        for location, per_sat in raw.items()
        for satellite, (dy, dx) in per_sat.items()
    }


def shift_chw(tensor: torch.Tensor, dy: float, dx: float) -> torch.Tensor:
    """Integer-pixel shift with edge padding (nearest-neighbor registration; sub-pixel
    remainder is dropped since the empirical/geometric shifts are themselves round-number
    estimates). Positive dy/dx follow the parallax convention: +dy=south, +dx=east."""
    if dy == 0 and dx == 0:
        return tensor
    idy, idx = int(round(dy)), int(round(dx))
    return torch.roll(tensor, shifts=(idy, idx), dims=(-2, -1))
...
class PrecipDataset(Dataset):
    def __init__(
        ...
        registration_shifts: dict[tuple[str, str], tuple[float, float]] | None = None,
    ) -> None:
        self.rows = rows
        ...
        self.registration_shifts = registration_shifts or {}
    ...
    def _observation_tensors(
        ...
        for slot in range(self.max_observations):
            ...
            dy, dx = self.registration_shifts.get((row["name_location"], expected_satellite), (0.0, 0.0))
            if dy or dx:
                raw = shift_chw(raw, dy, dx)
            maps.append(
                build_frame_tensor(
                    raw,
                    n_chan=n_chan,
