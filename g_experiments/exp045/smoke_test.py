#!/usr/bin/env python3
"""exp045 smoke test: config/channel consistency, CPU forward/backward for every arm, and a
...
def check_real_batch() -> None:
    ...
    registration_shifts = registration_shifts_from_config(config, lambda p: (SCRIPT_DIR / p).resolve())
    assert registration_shifts, "registration_shifts_path configured but table loaded empty"
    n_nonzero = sum(1 for dy, dx in registration_shifts.values() if dy or dx)
    print(f"registration table: {len(registration_shifts)} pairs, {n_nonzero} nonzero")

    # Search all rows (not just this fold's validation split) for a registered-Himawari
    # example -- this only exercises the shift mechanism, so the fold assignment is
    # irrelevant, and empirical_train registration is sparse enough that no single fold's
    # valid split is guaranteed to contain one.
    registered_row = next(
        (row for row in rows
         if registration_shifts.get((row["name_location"], row["satellite_target"]), (0.0, 0.0)) != (0.0, 0.0)),
        None,
    )
    assert registered_row is not None, "no row in train_dataset.csv matches a registered (location, satellite) pair"
    valid_rows = [registered_row] + valid_rows[:5]
    ...
    ds = PrecipDataset(valid_rows, train_dir, registration_shifts=registration_shifts, **ds_kwargs)
    ds_unregistered = PrecipDataset(valid_rows, train_dir, registration_shifts={}, **ds_kwargs)
    expected = int(config["model"]["in_channels"])

    himawari_row_seen = False
    for i in range(len(ds)):
        ...
        row = valid_rows[i]
        key = (row["name_location"], row["satellite_target"])
        if row["satellite_target"] == "himawari" and registration_shifts.get(key, (0.0, 0.0)) != (0.0, 0.0):
            himawari_row_seen = True
            registered = item["x"]
            unregistered = ds_unregistered[i]["x"]
            assert not torch.equal(registered, unregistered), (
                f"row {i} ({key}) has a nonzero registration shift but output is identical "
                "to the unregistered dataset -- shift not being applied"
            )
    assert himawari_row_seen, "expected registered_row to trigger the shift check but it didn't"
    model = build_model(config)
...
def main() -> None:
    for arm in ARMS:
        ...
        print(result)
    check_real_batch()
    print("exp045 smoke test PASSED")
