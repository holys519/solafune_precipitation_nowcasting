...
for fold in 0 1 2 3 4; do
  raw_job_id="$($SBATCH_BIN \
    --parsable \
    --job-name="exp045-fold${fold}" \
    --output="$SCRIPT_DIR/slurm-exp045-fold${fold}-%j.out" \
    --error="$SCRIPT_DIR/slurm-exp045-fold${fold}-%j.err" \
...
raw_submit_job_id="$($SBATCH_BIN \
  ...
  --job-name="exp045-submit" \
  --output="$SCRIPT_DIR/slurm-exp045-submit-%j.out" \
  --error="$SCRIPT_DIR/slurm-exp045-submit-%j.err" \
