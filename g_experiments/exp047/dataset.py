"""PyTorch datasets for exp047: exp017's full feature pipeline (canonical bands, engineered
...
from solar_features import N_SOLAR_CHANNELS, load_geocoded_locations, solar_position_channels
...
def features_from_config(config: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "solar_position": bool(features_cfg.get("solar_position", False)),
    }
...
def expected_in_channels(
    ...
    solar_channels = N_SOLAR_CHANNELS if features.get("solar_position") else 0
    return (frames * per_frame + frames + context_rows * channels_per_row(features)
            + len(SATELLITES) + solar_channels)


def geo_table_from_config(config: dict, resolve_path) -> dict[str, tuple[float, float]]:
    """Opt-in via config data.geocoded_locations_path (g_eda/exp005's frozen Nominatim
    table); absent or empty means solar_position must not be enabled."""
    raw_path = config.get("data", {}).get("geocoded_locations_path")
    if not raw_path:
        return {}
    return load_geocoded_locations(resolve_path(raw_path))
...
class PrecipDataset(Dataset):
    def __init__(
        ...
        geo_table: dict[str, tuple[float, float]] | None = None,
    ) -> None:
        self.rows = rows
    ...
    def _input_tensor(self, row: dict[str, str]) -> torch.Tensor:
        ...
        solar_maps: list[torch.Tensor] = []
        if self.features.get("solar_position"):
            lat, lon = self.geo_table[row["name_location"]]
            dt_utc = datetime.fromisoformat(row["datetime"])
            for value in solar_position_channels(lat, lon, dt_utc):
                solar_maps.append(torch.full((1, *self.target_size), float(value), dtype=torch.float32))

        return torch.cat(maps + masks + row_features + satellite_maps + solar_maps, dim=0)
