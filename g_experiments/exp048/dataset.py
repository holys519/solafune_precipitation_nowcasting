"""PyTorch datasets for exp048: exp017's full feature pipeline (canonical bands, engineered
...
def features_from_config(config: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "fog_btd": bool(features_cfg.get("fog_btd", False)),
    }
...
def fog_btd_channel(raw: torch.Tensor, satellite: str) -> torch.Tensor:
    """3.9um (mir) minus 10.5um (win) brightness-temperature-difference, the classic
    operational fog/low-cloud discriminator (doc/domain_knowledge_review_2026-07-20.md
    section 2.4): water-droplet clouds emit close to blackbody at 10.5um but well below
    blackbody at 3.9um, giving fog/stratus a distinctive BTD that convective cloud tops
    (optically thick at both wavelengths) do not share. Computed on raw uint8 values,
    same convention as frame_engineered_channels' other BTD-style features."""
    bands = KEY_BANDS[satellite]
    return ((raw[bands["mir"]] - raw[bands["win"]]) / DIFF_SCALE).unsqueeze(0)
...
def build_frame_tensor(
    ...
    if features.get("fog_btd"):
        parts.append(fog_btd_channel(raw, satellite))
