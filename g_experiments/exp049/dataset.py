"""PyTorch datasets for exp049: exp017's full feature pipeline (canonical bands, engineered
...
def features_from_config(config: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "ir_texture": bool(features_cfg.get("ir_texture", False)),
    }
...
IR_TEXTURE_SCALE = 16.0
IR_TEXTURE_KERNEL = 3


def ir_texture_channel(raw: torch.Tensor, satellite: str) -> torch.Tensor:
    """Local (3x3) standard deviation of the IR-window band on raw uint8 values -- the
    classic texture cue for convective vs. fog/stratiform discrimination
    (doc/domain_knowledge_review_2026-07-20.md section 2.4): fog/low stratus is spatially
    homogeneous (low local std), while convective cloud tops (esp. overshooting tops) are
    textured (high local std). Computed via the E[x^2]-E[x]^2 identity with avg_pool2d,
    edge-replicated padding to match the frame's own spatial size."""
    win = raw[KEY_BANDS[satellite]["win"]].float().unsqueeze(0).unsqueeze(0)
    pad = IR_TEXTURE_KERNEL // 2
    padded = F.pad(win, (pad, pad, pad, pad), mode="replicate")
    padded_sq = F.pad(win * win, (pad, pad, pad, pad), mode="replicate")
    mean = F.avg_pool2d(padded, IR_TEXTURE_KERNEL, stride=1)
    mean_sq = F.avg_pool2d(padded_sq, IR_TEXTURE_KERNEL, stride=1)
    std = (mean_sq - mean * mean).clamp_min(0.0).sqrt()
    return (std.squeeze(0) / IR_TEXTURE_SCALE)
...
def build_frame_tensor(
    ...
    if features.get("ir_texture"):
        parts.append(ir_texture_channel(raw, satellite))
