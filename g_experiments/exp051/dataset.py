"""PyTorch datasets for exp051: exp017's full feature pipeline (canonical bands, engineered
...
def features_from_config(config: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "gpi_fraction": bool(features_cfg.get("gpi_fraction", False)),
    }
...
GPI_THRESHOLD_FRACTION = 0.3


def gpi_fraction_channel(raw: torch.Tensor, satellite: str) -> torch.Tensor:
    """GOES Precipitation Index (GPI) style areal cold-cloud fraction: the fraction of this
    frame's pixels colder than a fixed threshold -- the classic, simplest IR-based rainfall
    estimator (doc/domain_knowledge_review_2026-07-20.md section 2.1). Broadcast as a
    constant scalar channel across the tile (like the satellite one-hot maps), since GPI is
    inherently an AREAL statistic, not a per-pixel one. Threshold placed 30% of the way from
    the per-satellite cold to warm anchor already used by ir_rain_proxy_channel, as a proxy
    for "notably cold, likely convective"."""
    ...
    threshold = cold_anchor + GPI_THRESHOLD_FRACTION * (warm_anchor - cold_anchor)
    win = raw[KEY_BANDS[satellite]["win"]]
    fraction = float((win <= threshold).float().mean())
    return torch.full((1, *win.shape), fraction, dtype=torch.float32)
...
def build_frame_tensor(
    ...
    if features.get("gpi_fraction"):
        parts.append(gpi_fraction_channel(raw, satellite))
