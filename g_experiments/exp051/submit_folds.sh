...
for fold in 0 1 2 3 4; do
  raw_job_id="$($SBATCH_BIN \
    --parsable \
    --job-name="exp051-fold${fold}" \
    --output="$SCRIPT_DIR/slurm-exp051-fold${fold}-%j.out" \
    --error="$SCRIPT_DIR/slurm-exp051-fold${fold}-%j.err" \
...
raw_submit_job_id="$($SBATCH_BIN \
  ...
  --job-name="exp051-submit" \
  --output="$SCRIPT_DIR/slurm-exp051-submit-%j.out" \
  --error="$SCRIPT_DIR/slurm-exp051-submit-%j.err" \
