"""PyTorch datasets for exp052: exp038's strict current-row-only pipeline (canonical bands,
engineered physics channels, temporal_abs, target_time_first, zero-observation row filter)
plus a NEW train/valid-only auxiliary future-frame target (ticket G-033, reinterpreted after
the 2026-07-20 organizer ruling).
...
- future_aux_head (NEW, this experiment): when enabled AND the dataset instance has
  has_target=True (train/valid only -- never evaluation), __getitem__ additionally looks up
  the SAME (name_location) row at datetime + 30*future_aux_horizon_rows minutes and returns
  its newest observation's IR-window band as a (1,H,W) auxiliary supervision target, plus a
  scalar validity flag (0 when that future row doesn't exist or belongs to a different
  satellite). This is a TRAIN-TIME-ONLY auxiliary signal for model.py's optional future_aux
  decoder head -- it never touches model inputs and is never computed at inference (see the
  has_target/evaluation_dir guards in `_future_aux_target` below, and the hard override in
  inference.py/make_submission.py). Legal per the 2026-07-20 ruling: "learning-time use of
  train-folder future frames as a supervision signal is fine (no leakage at inference time)."
"""
...
def features_from_config(config: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "future_aux_head": bool(features_cfg.get("future_aux_head", False)),
    }
...
def _augment(*tensors: torch.Tensor | None) -> tuple[torch.Tensor | None, ...]:
    """Applies the SAME random flip/rotation to every tensor passed in (None entries pass
    through unchanged) so x, y, and the future_aux target (when present) stay spatially
    consistent. Relies on the caller (train.py's worker_init_fn) having seeded the `random`
    module per-worker; uses the module-level RNG directly so augmentation varies across
    epochs instead of being fixed per sample index."""

    flip_h = random.random() < 0.5
    flip_v = random.random() < 0.5
    k = random.randint(0, 3)

    def transform(t: torch.Tensor | None) -> torch.Tensor | None:
        if t is None:
            return None
        if flip_h:
...
class PrecipDataset(Dataset):
    def __init__(
        ...
        self.future_aux_horizon_rows = int(future_aux_horizon_rows)
    ...
    def _future_aux_target(self, row: dict[str, str]) -> tuple[torch.Tensor, torch.Tensor]:
        """TRAIN/VALID-ONLY auxiliary supervision target (ticket G-033, reinterpreted per the
        2026-07-20 organizer ruling: "using future frames from the TRAIN split as a training-
        time auxiliary supervision signal is fine, since there is no leakage at inference
        time" -- doc/submission_registry.md item 4). Looks up the SAME (name_location) row at
        datetime + 30*future_aux_horizon_rows minutes via the same `_row_by_location_time`
        table already used for context_rows, and returns that row's newest observation's
        IR-window ("win") band, normalized the same way every other band is normalized.

        Returns ((1,H,W) target, scalar validity flag). Validity is 0.0 (and the target is an
        all-zero placeholder that must be excluded from the loss via the flag) when:
        - the row/location/datetime fields are unparsable,
        - no row exists at that future timestamp for this location (common at the end of a
          location's time series),
        - the future row is a different satellite than the current row,
        - the future row has no observations, or its newest observation is missing the win band.

        HARD GUARD (compliance requirement, see g_experiments/exp052/README.md): this method
        must only ever run against train/valid data. It raises if called on a dataset built
        with has_target=False (this codebase's evaluation-mode signal) or pointed at an
        evaluation data directory -- the future-target lookup path must never be reachable at
        inference time.
        """
        if not self.has_target:
            raise RuntimeError(
                "PrecipDataset._future_aux_target is train/valid-only and must never run when "
                "has_target=False (the evaluation-mode signal in this codebase); this is a "
                "hard compliance guard for the 2026-07-20 green ruling on train-time-only "
                "future-frame auxiliary supervision -- see g_experiments/exp052/README.md"
            )
        if "evaluation" in str(self.data_dir).lower():
            raise RuntimeError(
                "PrecipDataset._future_aux_target must never run against an evaluation data "
                f"directory (data_dir={self.data_dir!r}); this is a hard compliance guard -- "
                "see g_experiments/exp052/README.md"
            )

        zero_target = torch.zeros((1, *self.target_size), dtype=torch.float32)
        zero_valid = torch.tensor(0.0, dtype=torch.float32)
        try:
            location = row["name_location"]
            satellite = row["satellite_target"]
            start_time = datetime.fromisoformat(row["datetime"])
        except (KeyError, ValueError):
            return zero_target, zero_valid

        future_time = start_time + timedelta(minutes=30 * self.future_aux_horizon_rows)
        future_row = self._row_by_location_time.get((location, future_time))
        if future_row is None or future_row.get("satellite_target") != satellite:
            return zero_target, zero_valid

        try:
            obs_names = parse_observation_filenames(future_row["last_30_minutes_observation_filename"])
        except (ValueError, SyntaxError, KeyError):
            obs_names = []
        if not obs_names:
            return zero_target, zero_valid

        path = self.data_dir / satellite / obs_names[-1]
        if not path.exists():
            return zero_target, zero_valid
        raw, n_chan = read_satellite_raw(path, channels=self.satellite_channels, target_size=self.target_size)
        win_idx = KEY_BANDS[satellite]["win"]
        if win_idx >= n_chan:
            return zero_target, zero_valid  # win band was zero-padded on this frame

        normalized = normalize_raw(raw, n_chan, satellite, self.norm_stats)
        win_band = normalized[win_idx].unsqueeze(0).contiguous()
        return win_band, torch.tensor(1.0, dtype=torch.float32)
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        if self.has_target and self.features.get("future_aux_head"):
            future_target, future_valid = self._future_aux_target(row)

        if self.augment:
            x, y, future_target = _augment(x, y, future_target)
        ...
        if future_target is not None:
            item["future_target"] = future_target
