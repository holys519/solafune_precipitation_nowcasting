#!/usr/bin/env python3
"""Run GPU inference for exp052 and write prediction GeoTIFFs.
...
COMPLIANCE (hard requirement, see README.md): exp052 trains an optional train-time-only
auxiliary future-frame supervision head (features.future_aux_head / model.future_aux_head).
This module unconditionally forces both flags to False when building the evaluation dataset
and the serving model, regardless of what the training config says, and strips any
future_aux_decoder.* weights out of loaded checkpoints -- so the future-frame mechanism is
structurally absent from the serving graph, not just unused. See
`force_no_future_aux_features`/`force_no_future_aux_model_config` and `load_models` below.
"""
...
def force_no_future_aux_features(config: dict[str, Any]) -> dict[str, bool]:
    """COMPLIANCE (hard requirement): force features["future_aux_head"]=False for the
    evaluation dataset regardless of what the training config said, logging whenever this
    actually overrides something. Belt-and-suspenders -- has_target=False (always set for the
    evaluation dataset below) already makes PrecipDataset.__getitem__ skip the future-target
    lookup, and `_future_aux_target` independently raises if ever called with has_target=False
    -- but this flag is forced off explicitly too so there is no ambiguity for a code-inspection
    auditor.
    """
    eval_features = features_from_config(config)
    if eval_features.get("future_aux_head"):
        print(
            "COMPLIANCE OVERRIDE: config had features.future_aux_head=true; forcing False for "
            "the evaluation dataset -- the future-frame lookup must never run at inference "
            "time (2026-07-20 ruling; see README.md)",
            flush=True,
        )
    eval_features["future_aux_head"] = False
    return eval_features


def force_no_future_aux_model_config(config: dict[str, Any]) -> dict[str, Any]:
    """COMPLIANCE (hard requirement): serving must never build the future_aux decoder branch.

    Returns a deep-copied config with model.future_aux_head forced to False regardless of what
    the training config said, printing a log line whenever this actually overrides something.
    """
    eval_config = copy.deepcopy(config)
    was_enabled = bool(eval_config.get("model", {}).get("future_aux_head", False))
    if was_enabled:
        print(
            "COMPLIANCE OVERRIDE: config had model.future_aux_head=true; forcing False for "
            "serving/evaluation -- the future-frame auxiliary head is train-time-only and must "
            "never exist in the served model graph (2026-07-20 ruling; see README.md)",
            flush=True,
        )
    eval_config.setdefault("model", {})["future_aux_head"] = False
    return eval_config


def load_models(config: dict[str, Any], checkpoint_paths: list[Path]) -> list[nn.Module]:
    eval_config = force_no_future_aux_model_config(config)
    models = []
    for path in checkpoint_paths:
        checkpoint = torch.load(path, map_location="cpu", weights_only=False)
        model = build_model(eval_config)
        state_dict = checkpoint["model_state_dict"]
        # Drop any future_aux_decoder.* weights the checkpoint may hold (from a
        # future_aux_head=true training run) -- the eval model was built without that module,
        # so these keys would otherwise be reported "unexpected" by strict loading.
        dropped = [k for k in state_dict if k.startswith("future_aux_decoder.")]
        filtered_state_dict = {k: v for k, v in state_dict.items() if not k.startswith("future_aux_decoder.")}
        if dropped:
            print(
                f"COMPLIANCE OVERRIDE: dropping {len(dropped)} future_aux_decoder.* weight(s) "
                f"from {path} -- never loaded into the serving model",
                flush=True,
            )
        missing, unexpected = model.load_state_dict(filtered_state_dict, strict=False)
        if unexpected:
            raise RuntimeError(f"unexpected checkpoint keys after dropping future_aux_decoder.*: {unexpected}")
        if missing:
            raise RuntimeError(f"checkpoint is missing required (non-future_aux) keys: {missing}")
...
def main() -> None:
    ...
    eval_features = force_no_future_aux_features(config)
    ds = PrecipDataset(
        rows,
        eval_dir,
