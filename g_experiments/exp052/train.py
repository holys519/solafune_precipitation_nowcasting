#!/usr/bin/env python3
"""Train exp052: exp018's high-res localization net + exp028 temporal input design + exp030
dilated bottleneck, each gated by config so single axes can be switched off, plus a NEW
train-time-only auxiliary future-frame supervision head (features.future_aux_head /
loss.future_aux_weight, ticket G-033 reinterpreted per the 2026-07-20 organizer ruling --
see README.md). This is the only script that ever needs the future target: inference.py and
make_submission.py hard-force it off when serving."""
...
def main() -> None:
    ...
    analysis_dir = resolve_path(config["paths"].get("analysis_dir", "../../outputs/analysis/exp052"))
    ...
    train_ds = PrecipDataset(
        ...
        future_aux_horizon_rows=future_aux_horizon_rows,
    )
    ...
    experiment_name = str(config.get("experiment", {}).get("name", "exp052"))
    print(
    ...
    for epoch in range(1, epochs + 1):
        ...
        for step, batch in enumerate(train_loader, 1):
            x = batch["x"].to(device, non_blocking=True)
            y = batch["y"].to(device, non_blocking=True)
            # Only present in the batch when features.future_aux_head is enabled (train/valid
            # datasets only -- see dataset.py's PrecipDataset.__getitem__ and
            # _future_aux_target's has_target/evaluation_dir hard guards).
            future_target = batch.get("future_target")
            ...
            with cuda_autocast(enabled=bool(config["train"]["amp"])):
                ...
                loss = loss_fn(output, y, future_target=future_target, future_valid=future_valid)
