#!/bin/bash
# exp053 end-to-end entrypoint (no Slurm; run directly on a GPU box).
#
# Usage:
...
config_path() {
  ...
  # from the active config instead of hardcoding exp053's default paths.
  "$PYTHON" - "$CONFIG" "$1" <<'PY'
import sys, yaml
...
run_self_pred_oof() {
  # Gating criterion (doc/submission_registry.md): mimics real inference by substituting the
  # model's OWN prior prediction for the AR channel during validation, instead of teacher
  # forcing with ground truth. Only meaningful for fold0/fold4 checkpoints during the gate.
  for f in 0 4; do
    echo "== self-prediction-substituted OOF pass, fold $f =="
    "$PYTHON" self_pred_oof.py --config "$CONFIG" --fold "$f"
  done
}
...
if [ "$FOLD" = "all" ]; then
  ...
  run_inference
  run_submission
elif [ "$FOLD" = "self_pred_oof" ] || [ "$FOLD" = "gate" ]; then
  run_self_pred_oof
else
  run_fold "$FOLD"
fi
