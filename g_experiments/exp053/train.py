#!/usr/bin/env python3
"""Train exp053: exp038's high-res localization net (exp018 architecture + exp028 temporal
input design + exp030 dilated bottleneck) plus a new autoregressive own-past-prediction input
channel (features.autoregressive_prev_pred, see dataset.py). Teacher forcing is transparent to
this script: PrecipDataset sources the AR channel from ground truth automatically whenever
`ar_cache` is left at its default None (see dataset.py's `_autoregressive_prev_pred_channels`),
so no train.py changes were needed to support it."""
...
def main() -> None:
    ...
    experiment_name = str(config.get("experiment", {}).get("name", "exp053"))
    print(
