#!/usr/bin/env python3
"""Create fold logs and spatial OOF diagnostics for exp054."""
...
# exp054 amount-bin breakdown (README.md): fixed edges (mm), identical to losses.AMOUNT_BIN_EDGES
# and to l_eda/exp004/run_fold_anatomy.py's REGIME_EDGES, so results here line up directly with
# that EDA's regime tables and with the loss-reweighting mechanism being ablated.
#   b0=[0, 0.01)  b1=[0.01, 0.1)  b2=[0.1, 0.3)  b3=[0.3, 1.0)  b4=[1.0, 100]
AMOUNT_BIN_EDGES: tuple[float, ...] = (0.0, 0.01, 0.1, 0.3, 1.0, 100.0)
N_AMOUNT_BINS = len(AMOUNT_BIN_EDGES) - 1


def amount_bin_index(arr: np.ndarray) -> np.ndarray:
    """Bin index of each element of `arr` into AMOUNT_BIN_EDGES: edges[i] <= x < edges[i+1],
    with the last bin also catching x >= edges[-2]. Matches losses.amount_bin_index /
    l_eda/exp004's regime_of exactly (see those docstrings)."""
    idx = np.zeros_like(np.asarray(arr), dtype=np.int64)
    for i in range(1, N_AMOUNT_BINS):
        idx = np.where(arr >= AMOUNT_BIN_EDGES[i], i, idx)
    return idx


def empty_pixel_bin_stats() -> dict[int, dict[str, float]]:
    return {
        i: {"count": 0.0, "sse": 0.0, "sae": 0.0, "sum_target": 0.0, "sum_pred": 0.0}
        for i in range(N_AMOUNT_BINS)
    }


def update_pixel_bin_stats(stats: dict[int, dict[str, float]], pred_arr: np.ndarray, target_arr: np.ndarray) -> None:
    """Accumulate positive-pixel (target > 0) error stats, split by which AMOUNT_BIN_EDGES bin
    each pixel's TARGET value falls into. This is the pixel-level breakdown that answers
    "which amount regime does the model actually get wrong" (discussion/round6_findings_2026-07-19_ja.md
    section 3), as distinct from the tile-level breakdown (grouped[("amount_bin_tile_mean", ...)])
    which bins whole tiles by their overall wetness."""
    wet = target_arr > 0
    if not wet.any():
        return
    bin_idx = amount_bin_index(target_arr)
    ...
    for i in range(N_AMOUNT_BINS):
        ...
        if n == 0:
            continue
        d = diff[sel]
        stats[i]["count"] += float(n)
        stats[i]["sse"] += float(np.square(d).sum())
        stats[i]["sae"] += float(np.abs(d).sum())
        stats[i]["sum_target"] += float(target_arr[sel].sum())
        stats[i]["sum_pred"] += float(pred_arr[sel].sum())
...
def analyze_oof(config: dict[str, Any], checkpoint_paths: list[Path], analysis_dir: Path) -> dict[str, Any]:
    ...
    pixel_bin_stats = empty_pixel_bin_stats()
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        for batch in loader:
            ...
            for i, unique_id in enumerate(batch["unique_id"]):
                ...
                tile_amount_bin = int(amount_bin_index(np.asarray(target_arr.mean())))
                for key in [
                    ("fold", str(fold)),
                    ("location", location),
                    ("satellite", satellite),
                    ...
                    # exp054: tile-level amount regime, binned by AMOUNT_BIN_EDGES on the tile's
                    # OWN target_mean -- mirrors l_eda/exp004/run_fold_anatomy.py's regime_of
                    # grouping so oof_group_metrics.csv tile_rmse rows are directly comparable.
                    ("amount_bin_tile_mean", f"b{tile_amount_bin}"),
                ]:
                    grouped.setdefault(key, empty_stats())
                    update_stats(grouped[key], pred_arr, target_arr)
                # exp054: pixel-level breakdown -- bins each wet PIXEL by its own target value,
                # independent of the tile it sits in. This is what actually distinguishes the
                # midband vs. heavytail hypotheses (a tile can have target_mean in b0 while still
                # containing a handful of b4 pixels).
                update_pixel_bin_stats(pixel_bin_stats, pred_arr, target_arr)
    ...
    # exp054: the dedicated amount-regime table -- one row per bin of AMOUNT_BIN_EDGES, with both
    # the tile-level breakdown (tile_rmse over tiles whose OWN target_mean falls in the bin) and
    # the pixel-level breakdown (RMSE over wet pixels whose OWN target value falls in the bin).
    # This is the table that actually adjudicates midband vs. heavytail (README.md): compare each
    # bin's positive_pixel_rmse and positive_pixel_share_of_sse across config.yaml (control) vs.
    # config_midband.yaml vs. config_heavytail.yaml OOF runs.
    total_wet_pixels = sum(pixel_bin_stats[i]["count"] for i in range(N_AMOUNT_BINS))
    total_wet_sse = sum(pixel_bin_stats[i]["sse"] for i in range(N_AMOUNT_BINS))
    amount_bin_rows: list[dict[str, Any]] = []
    for i in range(N_AMOUNT_BINS):
        tile_stats = grouped.get(("amount_bin_tile_mean", f"b{i}"))
        pix = pixel_bin_stats[i]
        pix_count = pix["count"]
        amount_bin_rows.append(
            {
                "bin": f"b{i}",
                ...
                "positive_pixel_rmse": float(np.sqrt(pix["sse"] / pix_count)) if pix_count > 0 else None,
                "positive_pixel_mae": float(pix["sae"] / pix_count) if pix_count > 0 else None,
                "positive_pixel_share_of_sse": float(pix["sse"] / total_wet_sse) if total_wet_sse > 0 else None,
                "positive_pixel_mean_target": float(pix["sum_target"] / pix_count) if pix_count > 0 else None,
                "positive_pixel_mean_pred": float(pix["sum_pred"] / pix_count) if pix_count > 0 else None,
            }
        )
    amount_bin_path = analysis_dir / "oof_amount_bin_metrics.csv"
    with amount_bin_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=list(amount_bin_rows[0]))
        writer.writeheader()
        writer.writerows(amount_bin_rows)
    print(f"amount_bin_metrics (OOF, tile_rmse & positive-pixel RMSE by bin): {amount_bin_rows}", flush=True)
    ...
    summary = {
        ...
        "amount_bin_metrics_csv": str(amount_bin_path),
        "amount_bin_metrics": amount_bin_rows,
...
def main() -> None:
    ...
    analysis_dir = resolve_path(config["paths"].get("analysis_dir", "../../outputs/analysis/exp054"))
