#!/usr/bin/env python3
"""Build a seed-ensemble submission by averaging the per-tile eval predictions of exp056 (seed42),
exp056_seed123, and exp056_seed456. Variance-reduction ensemble of the champion architecture --
each member is the SAME green model (context_rows:1, mean×shape factorization), only the training
seed differs, so this is a low-inversion-risk average (see doc/oof_lb_transfer_by_category...).

Averages the 29090 float32 GeoTIFFs tile-by-tile, clips at 0, and writes each with the
corresponding exp056 tile as the metadata template (write_float32_like_template). Then zips
evaluation_target.csv + test_files/ in the standard submission layout.

Usage (inside the container):
    python3 build_seed_ensemble.py                       # 3-seed equal average
    python3 build_seed_ensemble.py --members exp056 exp056_seed123   # 2-seed
"""
