...
def features_from_config(config: dict) -> dict[str, bool]:
    ...
    return {
        ...
        "nodata_emissive_mask": bool(features_cfg.get("nodata_emissive_mask", False)),
    }
...
def load_norm_stats(path: Path) -> dict[str, dict[str, np.ndarray]]:
    ...
    for sat, values in raw.items():
        ...
        # exp059: per-band raw-scale fill value for emissive bands (NaN for reflective bands so
        # read_satellite_raw leaves their night-zeros alone). Only present when norm_stats.json
        # was produced by exp059's extended normalize_stats.py.
        if "emissive" in values:
            emissive = np.asarray(values["emissive"], dtype=np.float32)
            fill = np.where(emissive > 0.5, mean * 255.0, np.nan).astype(np.float32)
            entry["emissive_fill"] = fill
        stats[sat] = entry
    return stats
...
def read_satellite_raw(
    ...
    emissive_fill: np.ndarray | None = None,
) -> tuple[torch.Tensor, int, bool]:
    """Read a frame at RAW uint8 scale (0..255), resized, zero-padded to `channels`.

    Returns (tensor, n_chan_present, all_zero_frame). The raw scale is what the engineered
    physics channels must be computed on (uint8 quantization kills differences taken after
    normalization).

    exp059 NODATA handling (peppamint's discussion finding): a raw 0 in an emissive (IR/WV) band
    is a NODATA fill value, and left as-is it normalizes to a -2.5..-6 sigma "false cold cloud".
    We replace those exact-0 emissive pixels with the band's mean raw value HERE, on the NATIVE
    raster BEFORE resize, so the cold-fill contamination cannot bleed into neighbours during the
    antialiased downsize (detecting zeros after resize would both miss partially-blended fill and
    create a halo). Reflective/visible-band zeros (night) are deliberately left untouched
    (emissive_fill is NaN for those bands). all_zero_frame flags a frame that is exactly 0 across
    every band (a fully-clouded/missing observation), which the caller treats as an absent slot.
    """
    arr, _ = read_tiff_array(path)
    if arr.ndim == 2:
        arr = arr[:, :, None]
    n_chan = min(arr.shape[2], channels)
    arr = np.moveaxis(arr[:, :, :n_chan].astype(np.float32), -1, 0)  # (C, H, W)
    all_zero_frame = bool(np.all(arr == 0))
    if emissive_fill is not None and not all_zero_frame:
        for b in range(n_chan):
            fill = emissive_fill[b]
            if np.isfinite(fill):  # finite only for emissive bands
                band = arr[b]
                band[band == 0] = fill
    ...
    return _resize_chw(tensor, target_size), n_chan, all_zero_frame
...
class PrecipDataset(Dataset):
    ...
    def _observation_tensors(
        ...
        for slot in range(self.max_observations):
            ...
            emissive_fill = None
            if self.features.get("nodata_emissive_mask") and self.norm_stats is not None:
                sat_stats = self.norm_stats.get(expected_satellite)
                if sat_stats is not None:
                    emissive_fill = sat_stats.get("emissive_fill")
            raw, n_chan, all_zero_frame = read_satellite_raw(
                self.data_dir / expected_satellite / obs_name,
                channels=self.satellite_channels,
                target_size=self.target_size,
                emissive_fill=emissive_fill,
            )
            if self.features.get("nodata_emissive_mask") and all_zero_frame:
                # Fully-clouded / all-zero observation: a uniform cold field posing as a real
                # frame. Treat as an absent slot (zeros + mask 0), exactly like a missing
                # observation, rather than feeding the network confident noise (peppamint §"all-zero
                # frames"). raw_frames None so it also drops out of the temporal-diff feature.
                maps.append(torch.zeros((self._frame_channels, *self.target_size), dtype=torch.float32))
                masks.append(torch.zeros((1, *self.target_size), dtype=torch.float32))
                raw_frames.append(None)
                continue
