...
`HurdleLogNormalLoss` and the control-arm losses below are copied UNCHANGED from exp038 for
reference/compatibility with `architecture: highres_hurdle_lognormal_unet` etc. The new
`FactorizedMeanShapeLoss` (loss.name: factorized_mean_shape) is this experiment's own loss,
matching `FactorizedMeanShapeUNet` in model.py. See README.md for the full rationale.
"""
...
class FactorizedMeanShapeLoss(nn.Module):
    """Loss for `FactorizedMeanShapeUNet`: occurrence BCE (unchanged from exp038's hurdle loss) +
    two new terms replacing the joint per-pixel (mu, sigma) log-normal NLL:

    - mean_intensity_weight: log1p-space MSE between the predicted scalar `mean_intensity` and the
      tile's TRUE full-tile mean of the target (`target.mean(dim=(1,2,3))` -- matches g_eda/exp002's
      `amount_swap` target exactly, see model.py docstring). log1p keeps this in the same spirit as
      exp038's log-space intensity head while staying finite at target_mean=0 (dry tiles), unlike a
      plain log(). Set to 0 to ablate this term independently (mean_intensity head then gets no
      gradient from this loss -- still well-defined, no crash).
    - shape_weight: masked MSE between the predicted normalized `shape` field and the true field
      normalized the same way (`target / (target.mean() + shape_eps)`), evaluated ONLY at wet
      pixels (target > rain_threshold) exactly like exp038's intensity NLL is wet-only. All-dry
      tiles contribute zero wet pixels and are therefore automatically zero-weighted -- no NaN,
      no explicit per-tile branch needed. Set to 0 to ablate independently.

    aux_mask_weight / multiscale_weight_2 / multiscale_weight_4 are carried over unchanged from
    exp038's HurdleLogNormalLoss (same config keys, same semantics) so the auxiliary-supervision
    budget matches exactly between exp038 and exp056.
    """
    ...
    def __init__(
        ...
        rain_threshold: float = 0.0,
        bce_weight: float = 1.0,
        mean_intensity_weight: float = 1.0,
        ...
        self.rain_threshold = float(rain_threshold)
        self.bce_weight = float(bce_weight)
        self.mean_intensity_weight = float(mean_intensity_weight)
    ...
    def forward(self, output: dict[str, torch.Tensor], target: torch.Tensor) -> torch.Tensor:
        if not isinstance(output, dict) or "mean_intensity" not in output or "shape" not in output:
            raise TypeError("factorized_mean_shape loss requires the FactorizedMeanShapeUNet output dict")
        target = target.float()
        rain_mask = target > self.rain_threshold
        rain_target = rain_mask.float()
        bce = F.binary_cross_entropy_with_logits(output["rain_logits"].float(), rain_target)
        total = self.bce_weight * bce

        # 2026-07-24 bugfix (see README.md "Known bug"): mean_intensity/shape must be trained
        # against the WET-PIXEL-CONDITIONAL mean E[amount | wet], not the full-tile (dry-diluted)
        # mean. The served amount is `rain_prob * mean_intensity * shape` -- an explicit hurdle
        # gate -- exactly mirroring exp038's `rain_prob * exp(mu + sigma^2/2)`, where mu/sigma are
        # likewise fit only on wet pixels. Supervising mean_intensity against the full-tile mean
        # while still gating by rain_prob at serving time double-dilutes wet-pixel predictions by
        # roughly 1/target_positive_ratio (~5.6x too small at target_positive_ratio~0.18), which
        # is what caused exp056's fold0/4 gate failure (both folds +0.025..+0.033 tile_rmse).
        wet_pixel_count = rain_mask.float().sum(dim=(1, 2, 3)).clamp_min(1.0)
        wet_pixel_sum = (target * rain_mask.float()).sum(dim=(1, 2, 3))
        wet_mean = wet_pixel_sum / wet_pixel_count  # 0 for all-dry tiles (sum=0), as intended

        if self.mean_intensity_weight > 0:
            pred_mean_intensity = output["mean_intensity"].float().view(-1)
            mean_intensity_loss = F.mse_loss(
                torch.log1p(pred_mean_intensity), torch.log1p(wet_mean.clamp_min(0.0))
            )
            total = total + self.mean_intensity_weight * mean_intensity_loss

        if self.shape_weight > 0:
            wet = rain_mask
            if wet.any():
                shape_target = target / (wet_mean.view(-1, 1, 1, 1) + self.shape_eps)
                shape_pred = output["shape"].float()
                shape_loss = torch.square(shape_pred - shape_target)[wet].mean()
            else:
                shape_loss = torch.zeros((), device=target.device)
...
class FactorizedTotalDistributionLoss(nn.Module):
    """exp060 loss for `FactorizedTotalDistributionUNet`. The two primary terms are both BOUNDED,
    which is the fix for exp058's raw-mm-space NaN blow-up:

    - total_weight: MSE in LOG1P space between the model's `log_total` and log1p(true tile total),
      where true tile total = target.sum() per tile. Log space keeps gradients bounded even for
      heavy-rain tiles whose totals reach the hundreds -- exactly how exp056/exp038 train their
      intensity heads. (exp058 instead put the main weight on raw-mm prediction MSE with an
      unbounded softplus total; that is what diverged to NaN.)
    - distribution_weight: MSE between the predicted softmax `distribution` (sums to 1) and the true
      normalized field target/target.sum() (also sums to 1), evaluated only on WET tiles. All-dry
      tiles have an undefined normalized field and are zero-weighted (no NaN, no per-tile branch).

    occurrence BCE is auxiliary (it never gates the served amount). aux_mask / multiscale terms are
    carried over unchanged from exp056 for auxiliary-supervision parity; multiscale acts on the
    served prediction, which is now bounded (total is log-clamped, distribution in [0,1]), so it
    cannot reintroduce the blow-up. Any term with weight 0 is skipped cleanly.
    """
    ...
    def forward(self, output: dict[str, torch.Tensor], target: torch.Tensor) -> torch.Tensor:
        if not isinstance(output, dict) or "distribution" not in output or "log_total" not in output:
            raise TypeError("factorized_total_distribution loss requires the FactorizedTotalDistributionUNet output dict")
        target = target.float()
        rain_target = (target > self.rain_threshold).float()
        bce = F.binary_cross_entropy_with_logits(output["occurrence_logits"].float(), rain_target)
        total = self.bce_weight * bce

        tile_total = target.sum(dim=(1, 2, 3))  # (B,) true tile total precip

        if self.total_weight > 0:
            log_total_pred = output["log_total"].float().view(-1)
            total_loss = F.mse_loss(log_total_pred, torch.log1p(tile_total.clamp_min(0.0)))
            total = total + self.total_weight * total_loss

        if self.distribution_weight > 0:
            wet_tile = tile_total > 0
            if wet_tile.any():
                true_dist = target / (tile_total.view(-1, 1, 1, 1) + self.dist_eps)
                pred_dist = output["distribution"].float()
                per_tile = torch.square(pred_dist - true_dist).mean(dim=(1, 2, 3))
                dist_loss = per_tile[wet_tile].mean()
            else:
                dist_loss = torch.zeros((), device=target.device)
            total = total + self.distribution_weight * dist_loss
...
def build_loss(config: dict) -> nn.Module:
    ...
    if name == "factorized_total_distribution":
        return FactorizedTotalDistributionLoss(
            rain_threshold=float(loss_cfg.get("rain_threshold", 0.0)),
            bce_weight=float(loss_cfg.get("bce_weight", 1.0)),
            total_weight=float(loss_cfg.get("total_weight", 1.0)),
            distribution_weight=float(loss_cfg.get("distribution_weight", 1.0)),
            dist_eps=float(loss_cfg.get("dist_eps", 1e-6)),
    ...
    if name == "factorized_mean_shape":
        return FactorizedMeanShapeLoss(
            rain_threshold=float(loss_cfg.get("rain_threshold", 0.0)),
            bce_weight=float(loss_cfg.get("bce_weight", 1.0)),
            mean_intensity_weight=float(loss_cfg.get("mean_intensity_weight", 1.0)),
            shape_weight=float(loss_cfg.get("shape_weight", 1.0)),
            shape_eps=float(loss_cfg.get("shape_eps", 1e-6)),
            aux_mask_weight=float(loss_cfg.get("aux_mask_weight", 0.0)),
            aux_mask_threshold=float(loss_cfg.get("aux_mask_threshold", 0.25)),
            aux_dice_weight=float(loss_cfg.get("aux_dice_weight", 0.5)),
            multiscale_weight_2=float(loss_cfg.get("multiscale_weight_2", 0.0)),
