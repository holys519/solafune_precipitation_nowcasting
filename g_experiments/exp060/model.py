...
class FactorizedTotalDistributionUNet(nn.Module):
    """exp060: the exact total x spatial-distribution factorization (exp058's intent) rebuilt on
    exp056's proven stable encoder/decoder and, critically, exp056's log-space training discipline.

    vs exp056's FactorizedMeanShapeUNet (the current champion):
    - Distribution is a softmax over the whole 41x41 tile, so it sums to EXACTLY 1 and therefore
      sum(prediction) == tile_total exactly -- a mathematically identifiable decomposition, with no
      separate rain_prob gate multiplying the served amount (occurrence is auxiliary only). This is
      exp058's "amount-identifiable" design.
    - Total is a single scalar per tile, parameterized in LOG space: the MLP emits `log_total`
      (clamped for numerical safety only), and total = expm1(log_total). This is the fix for
      exp058's NaN blow-up: exp058 applied an unbounded softplus total and put the main loss weight
      on a RAW-mm-space prediction MSE, so heavy-rain tiles (up to ~150 mm/pixel, tile totals in the
      hundreds) produced exploding squared errors and gradients -> NaN by epoch ~12 on both folds.
      Here the total is supervised in log1p space (bounded gradients, exactly how exp056/exp038's
      intensity heads are trained -- this project's repeatedly-confirmed "log space wins" rule), and
      the served prediction never carries the dominant loss weight in raw mm space.

    Heads: spatial_head -> distribution logits; total_mlp -> log_total scalar; occurrence_head
    (auxiliary BCE only, never gates the served amount); optional aux_mask_head (unchanged from
    exp056 for auxiliary-budget parity). Encoder/decoder/`head()` are identical to
    FactorizedMeanShapeUNet so this stays a clean single-axis change (formulation only) at the same
    internal resolution (128); a higher-resolution variant is a deliberate separate follow-up.
    """
    ...
    def __init__(
        ...
        self.output_size = tuple(int(v) for v in output_size)
        self.aux_mask_enabled = bool(aux_mask_enabled)
        # expm1(13) ~ 4.4e5: far above any real tile total (hundreds), so the clamp only prevents
        # fp overflow and never binds a plausible prediction. Kept well under fp16's ~6.5e4 overflow
        # by clamping BEFORE expm1 and computing expm1 in fp32.
        self.log_total_max = float(log_total_max)
        ...
        self.spatial_head = head()
        self.occurrence_head = head()
        if self.aux_mask_enabled:
            self.aux_mask_head = head()
        self.total_mlp = nn.Sequential(
            nn.Linear(c, total_hidden),
            nn.SiLU(inplace=True),
    ...
    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        spatial_logits = self._native(self.spatial_head(features)).float()  # (B,1,H,W)
        distribution = torch.softmax(spatial_logits.flatten(1), dim=1).view_as(spatial_logits)

        occurrence_logits = self._native(self.occurrence_head(features))
        occurrence_prob = torch.sigmoid(occurrence_logits.float())

        pooled = features.float().mean(dim=(2, 3))  # global average pool over the internal grid
        log_total = self.total_mlp(pooled).float().clamp(max=self.log_total_max)  # (B,1)
        total = torch.expm1(log_total).clamp_min(0.0)  # (B,1), non-negative

        prediction = total.view(b, 1, 1, 1) * distribution
        output = {
            "pred": prediction,
            "distribution": distribution,
            "tile_total": total.view(b),
            "log_total": log_total.view(b),
...
def build_model(config: dict) -> nn.Module:
    ...
    if architecture == "factorized_total_distribution":
        return FactorizedTotalDistributionUNet(
            in_channels=in_channels,
            base_channels=int(model_cfg.get("base_channels", 32)),
            internal_size=int(model_cfg.get("internal_size", 128)),
            output_size=(int(config["data"]["target_height"]), int(config["data"]["target_width"])),
            aux_mask_enabled=bool(model_cfg.get("aux_mask_enabled", True)),
            total_hidden=int(model_cfg.get("total_hidden", 64)),
            log_total_max=float(model_cfg.get("log_total_max", 13.0)),
            bottleneck_dilations=tuple(model_cfg.get("bottleneck_dilations", []) or []),
        )
