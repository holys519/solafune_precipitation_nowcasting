...
class PrecipDataset(Dataset):
    ...
    def _context_rows(self, row: dict[str, str]) -> list[dict[str, str] | None]:
        """exp063: LEGAL long causal history. context_rows counts the current row plus
        (context_rows-1) PREDECESSOR rows at T-30, T-60, ... (minus timedelta), all strictly
        earlier than the target time T. This is the opposite direction from exp009/016/017's
        now-banned SUCCESSOR (T+30, future) rows: the organizers' final ruling explicitly permits
        unlimited PAST history (T-40, T-50, T-60, ...). Every appended row's own observations are
        timestamped <= its datetime <= T-30 < T, so nothing at or after T enters the input.
        Must be re-verified by scripts/verify_causal_replay.py before any submission."""
