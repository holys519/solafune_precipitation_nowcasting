#!/usr/bin/env python3
"""Create fold logs and spatial OOF diagnostics for exp056.

Extends exp038's analyze_oof.py with an oracle-ladder sanity check (see g_eda/exp002's
run_oracle_ladder.py `amount_swap` counterfactual): per fold and pooled, in addition to the
standard tile_rmse, we recompute tile_rmse after rescaling THIS model's own served prediction so
its full-tile mean matches the TRUE full-tile mean (i.e. "perfect amount, our own spatial
placement/shape"). If the mean-intensity x shape factorization is wired correctly this should land
close to the historical amount_swap rung (~0.54-0.55 on this architecture family; exp018's own
amount_swap was 0.5446, exp038's was 0.5534 -- see outputs/g_eda/exp002/*_oracle_ladder.json)."""
...
def amount_swap_tile_rmse(pred_arr: np.ndarray, target_arr: np.ndarray) -> float:
    """Per-tile oracle-ladder sanity check (g_eda/exp002's `amount_swap` counterfactual,
    reimplemented here on THIS model's own OOF predictions): rescale the served prediction so its
    full-tile mean matches the TRUE full-tile mean, keeping the model's own spatial
    placement/shape untouched, then score tile_rmse against the true field.

    `pred_arr.mean()` / `target_arr.mean()` are full-tile means over all H*W pixels (matching
    `truth.mean(dim=(1,2,3))` / `pred.mean(dim=(1,2,3))` in run_oracle_ladder.py's `ladder_metrics`
    exactly -- NOT a wet-pixel-only mean). Dry-predicted tiles (pred_mean ~ 0, no spatial
    information to rescale) fall back to the flat true-mean oracle, mirroring the reference
    script's `torch.where(pred_mean > 1e-6, ...)` branch.
    """
    pred_mean = float(pred_arr.mean())
    target_mean = float(target_arr.mean())
    if pred_mean > 1e-6:
        swapped = pred_arr * (target_mean / pred_mean)
    else:
        swapped = np.full_like(target_arr, target_mean)
    return float(np.sqrt(np.square(swapped - target_arr).mean()))
...
def analyze_oof(config: dict[str, Any], checkpoint_paths: list[Path], analysis_dir: Path) -> dict[str, Any]:
    ...
    # Oracle-ladder sanity check (amount_swap counterfactual, see amount_swap_tile_rmse docstring).
    oracle_sums: dict[tuple[str, str], float] = {}
    oracle_counts: dict[tuple[str, str], int] = {}
    ...
    for checkpoint_path in checkpoint_paths:
        ...
        for batch in loader:
            ...
            for i, unique_id in enumerate(batch["unique_id"]):
                ...
                spatial_threshold = float(config.get("loss", {}).get("aux_mask_threshold", 0.25))
                pred_wet = pred_arr >= spatial_threshold
                target_wet = target_arr >= spatial_threshold
                union = np.logical_or(pred_wet, target_wet).sum()
                wet_iou = float(np.logical_and(pred_wet, target_wet).sum() / union) if union else 1.0
                pred_flat = pred_arr.ravel()
                target_flat = target_arr.ravel()
                if pred_flat.std() > 0 and target_flat.std() > 0:
                    spatial_corr = float(np.corrcoef(pred_flat, target_flat)[0, 1])
                else:
                    spatial_corr = float("nan")
                yy, xx = np.indices(target_arr.shape)
                pred_mass = float(pred_arr.sum())
                target_mass = float(target_arr.sum())
                if pred_mass > 0 and target_mass > 0:
                    pred_center = np.array([(yy * pred_arr).sum(), (xx * pred_arr).sum()]) / pred_mass
                    target_center = np.array([(yy * target_arr).sum(), (xx * target_arr).sum()]) / target_mass
                    centroid_distance = float(np.linalg.norm(pred_center - target_center))
                else:
                ...
                amount_swap_rmse = amount_swap_tile_rmse(pred_arr, target_arr)
                group_keys = [
                    ("fold", str(fold)),
                    ("location", location),
                    ("satellite", satellite),
                ...
                for key in group_keys + [("global", "oof")]:
                    oracle_sums[key] = oracle_sums.get(key, 0.0) + amount_swap_rmse
                ...
                sample_rows.append(
                    {
                        ...
                        "target_wet_ratio_025": float(target_wet.mean()),
                        "pred_wet_ratio_025": float(pred_wet.mean()),
                        "amount_swap_tile_rmse": amount_swap_rmse,
                    }
                )
    ...
    # Oracle-ladder sanity check (amount_swap counterfactual). group_rows already carries the
    # actual tile_rmse per group; join in amount_swap_tile_rmse so both numbers sit side by side.
    actual_tile_rmse_by_key = {(row["group_type"], row["group_value"]): row["tile_rmse"] for row in group_rows}
    oracle_ladder_rows = []
    for key, total in oracle_sums.items():
        group_type, group_value = key
        n = oracle_counts[key]
        amount_swap_mean = total / n
        oracle_ladder_rows.append(
            {
                "group_type": group_type,
                "group_value": group_value,
                "samples": n,
                "actual_tile_rmse": actual_tile_rmse_by_key.get(key),
                "amount_swap_tile_rmse": amount_swap_mean,
                "placement_headroom": (
                    (actual_tile_rmse_by_key[key] - amount_swap_mean) if key in actual_tile_rmse_by_key else None
                ),
            }
        )
    oracle_ladder_path = analysis_dir / "oof_oracle_ladder.csv"
    if oracle_ladder_rows:
        with oracle_ladder_path.open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(oracle_ladder_rows[0]))
            writer.writeheader()
            writer.writerows(sorted(oracle_ladder_rows, key=lambda row: (row["group_type"], row["group_value"])))
    ...
    # Oracle-ladder sanity check (task requirement 3): does swapping in the TRUE tile mean
    # intensity (keeping THIS model's own predicted shape) land near the historical amount_swap
    # rung? Reference points, both from `outputs/g_eda/exp002/*_oracle_ladder.json`
    # (run_oracle_ladder.py, same amount_swap formula reimplemented in amount_swap_tile_rmse()):
    #   - full 5-fold global pooled: exp018 amount_swap=0.5446, exp038 amount_swap=0.5534
    #   - fold0+fold4 ONLY pooled (fold0/fold4 are much easier than the OOF-wide average -- see
    #     doc/domain_knowledge_review_2026-07-20.md), a fairer reference for a fold0/4-only gate:
    #     exp038 fold0=0.24878 (n=8590) / fold4=0.54929 (n=8737) -> weighted amount_swap ~0.400,
    #     vs exp038 fold0/4 actual tile_rmse (weighted) ~0.444.
    global_key = ("global", "oof")
    global_amount_swap = oracle_sums.get(global_key, 0.0) / oracle_counts[global_key] if global_key in oracle_counts else None
    global_actual = global_row.get("tile_rmse")
    placement_headroom = (global_actual - global_amount_swap) if global_amount_swap is not None else None
    is_fold0_fold4_only = {checkpoint_fold(p) for p in checkpoint_paths} == {0, 4}
    reference_amount_swap = 0.400 if is_fold0_fold4_only else 0.5534
    reference_note = (
        "fold0+fold4-only pooled reference (exp038: ~0.400)"
        if is_fold0_fold4_only
        else "full 5-fold global reference (exp038: 0.5534, exp018: 0.5446)"
    )
    sanity_flag = None
    if global_amount_swap is not None:
        deviation = global_amount_swap - reference_amount_swap
        if abs(deviation) > 0.08:
            sanity_flag = (
                f"FLAG: global amount_swap_tile_rmse={global_amount_swap:.4f} deviates by "
                f"{deviation:+.4f} from the {reference_note}={reference_amount_swap:.4f} by more "
                "than the 0.08 tolerance -- check mean_intensity/shape head wiring before trusting "
                "this run's tile_rmse comparison to exp038."
            )
        else:
            sanity_flag = (
                f"OK: global amount_swap_tile_rmse={global_amount_swap:.4f} is within 0.08 of the "
                f"{reference_note}={reference_amount_swap:.4f}."
            )
        print(sanity_flag, flush=True)
    ...
    summary = {
        ...
        "oracle_ladder": {
            "oracle_ladder_csv": str(oracle_ladder_path) if oracle_ladder_rows else None,
            "global_actual_tile_rmse": global_actual,
            "global_amount_swap_tile_rmse": global_amount_swap,
            "placement_headroom": placement_headroom,
            "is_fold0_fold4_only": is_fold0_fold4_only,
            "reference_amount_swap": reference_amount_swap,
...
def main() -> None:
    ...
    analysis_dir = resolve_path(config["paths"].get("analysis_dir", "../../outputs/analysis/exp056"))
