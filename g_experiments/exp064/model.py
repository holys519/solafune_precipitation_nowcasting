...
class PretrainedFactorizedUNet(nn.Module):
    """exp064: exp056's mean-intensity x normalized-shape factorization head, but on a timm
    ImageNet-PRETRAINED encoder + a U-Net decoder, instead of exp056's from-scratch CompactUNet.

    Rationale (doc/twin_fold_and_trap_audit / this session): the from-scratch ~5M-param CNN was
    chosen very early -- partly on a since-resolved worry that ImageNet weights count as prohibited
    external data (the final ruling permits licensed public pretrained weights), partly because a
    54-channel non-RGB stem can't use RGB weights out of the box, and it was then deferred (ticket
    L-002, never started) and forgotten. timm auto-adapts the stem for `in_chans=54` and the weights
    download in-container, so this is now a clean test of the "from-scratch is underpowered"
    hypothesis. Encoder is config-selectable so all five candidates run in parallel.

    Output dict is byte-for-byte the same interface as FactorizedMeanShapeUNet (pred / rain_logits /
    rain_prob / shape / shape_raw / mean_intensity / rain_amount / aux_mask_logits), including the
    2026-07-24 wet-conditional bugfix, so losses.py (factorized_mean_shape) and analyze_oof.py work
    unchanged. Only the feature extractor differs -- a clean single-axis change vs the champion.

    License note (record in manifest before any submission): timm ImageNet weights for the
    recommended encoders are Apache-2.0 (efficientnet*, efficientnetv2, resnet) / MIT (convnext,
    swin). Distributed-data-only otherwise; no external satellite data.
    """
    ...
    def __init__(
        ...
        self.lateral = nn.ModuleList(nn.Conv2d(ch, c, kernel_size=1) for ch in enc_chs)
        self.fuse = nn.ModuleList(ConvBlock(2 * c, c) for _ in range(len(enc_chs) - 1))
        self.deep_channels = enc_chs[-1]
        ...
        if self.aux_mask_enabled:
            self.aux_mask_head = head()
        self.mean_intensity_mlp = nn.Sequential(
            nn.Linear(self.deep_channels, mean_intensity_hidden),
            nn.SiLU(inplace=True),
            nn.Linear(mean_intensity_hidden, 1),
        )
    ...
    def forward_features(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(len(laterals) - 2, -1, -1):
            skip = laterals[i]
            d = self.fuse[i](torch.cat([self._up(d, skip), skip], dim=1))
        return d, deep_pooled  # d at the shallowest encoder stride

    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        features, deep_pooled = self.forward_features(x)
        rain_logits = self._native(self.rain_head(features))
        rain_prob = torch.sigmoid(rain_logits.float())
        ...
        # tile mean-intensity from the deepest encoder features' global pool (wet-conditional target)
        mean_intensity = F.softplus(self.mean_intensity_mlp(deep_pooled).float()).view(-1, 1, 1, 1)
...
def build_model(config: dict) -> nn.Module:
    ...
    if architecture == "pretrained_factorized":
        return PretrainedFactorizedUNet(
            encoder_name=str(model_cfg.get("encoder_name", "efficientnet_b3")),
            in_channels=in_channels,
            internal_size=int(model_cfg.get("internal_size", 160)),
            output_size=(int(config["data"]["target_height"]), int(config["data"]["target_width"])),
            aux_mask_enabled=bool(model_cfg.get("aux_mask_enabled", True)),
            mean_intensity_hidden=int(model_cfg.get("mean_intensity_hidden", 64)),
            shape_eps=float(model_cfg.get("shape_eps", 1e-6)),
