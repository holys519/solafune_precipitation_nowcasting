"""PyTorch datasets for exp056 (copied unmodified from exp038): exp017's full feature pipeline (canonical bands, engineered
...
def expected_in_channels(
    ...
    base = frames * per_frame + frames + context_rows * channels_per_row(features) + len(SATELLITES)
...
def load_prev_pred_lookup(npz_path: Path, all_rows: list[dict[str, str]]) -> dict[tuple[str, datetime], np.ndarray]:
    """Build a (name_location, datetime) -> prediction array lookup for the fixed autoregressive
    feature, sourced from an existing OOF/nested-blend prediction cache (unique_id-keyed npz with
    "unique_id" and "pred" arrays, e.g. g_eda/exp011/nested_blend.py's
    <name>_oof_pred.npz output).

    This is exp067's fix for exp053's exposure-bias failure (doc/submission_registry.md,
    2026-08-01 audit): exp053 fed the TRUE ground-truth GPM value as the "previous prediction"
    input channel during training, then had to switch to its own (much worse) self-generated
    predictions at eval time -- a train/inference input-quality mismatch that collapsed accuracy
    (fold0 tile_rmse 0.199 teacher-forced vs 0.614 self-prediction-substituted). Here the
    channel is ALWAYS sourced from a realistic, already-imperfect held-out model prediction, at
    both train and eval time, so the model never sees an input quality it won't get in
    production. The nested-blend OOF cache is fold-consistent with this project's
    `make_group_kfold_split` (same seed/method), so for any row R being trained on, the
    previous-row P's cached prediction was produced by a model that never saw P's own
    fold -- and since P is always at the same location as R (same fold), it never saw R's
    fold either. No leakage into R's own label, at train OR validation time.
    """
    data = np.load(npz_path, allow_pickle=True)
    pred_by_id = {str(uid): arr for uid, arr in zip(data["unique_id"], data["pred"])}
    lookup: dict[tuple[str, datetime], np.ndarray] = {}
    missing = 0
    for row in all_rows:
        uid = row.get("unique_id")
        arr = pred_by_id.get(str(uid)) if uid is not None else None
        if arr is None:
            missing += 1
            continue
        try:
        ...
        lookup[key] = arr
    print(
        f"load_prev_pred_lookup: {len(lookup)}/{len(all_rows)} rows resolved from {npz_path} "
        f"({missing} missing)",
        flush=True,
    )
    return lookup
...
class PrecipDataset(Dataset):
    def __init__(
        ...
        prev_pred_lookup: dict[tuple[str, datetime], np.ndarray] | None = None,
    ) -> None:
        self.rows = rows
        ...
        # features.autoregressive_prev_pred sourcing (exp067 fix for exp053's exposure bias --
        # see load_prev_pred_lookup's docstring): ar_cache (eval / self-prediction-substituted
        # OOF gate) takes priority when set; otherwise prev_pred_lookup (train/valid, a FIXED
        # precomputed realistic OOF prediction -- never ground truth). Neither set => zero+mask=0.
        self.ar_cache = ar_cache
        self.prev_pred_lookup = prev_pred_lookup
    ...
    def _autoregressive_prev_pred_channels(self, row: dict[str, str]) -> list[torch.Tensor]:
        ...
        再利用(自己回帰/recursive手法)"). Unlike exp053 (doc/submission_registry.md's 2026-08-01
        audit: exposure bias, teacher-forced-on-ground-truth training collapsed at real
        self-prediction-substituted eval), the channel here is NEVER sourced from ground truth:

        - self.ar_cache is not None (eval / self-prediction-substituted OOF gate): the model's
          own just-computed prediction, populated by the caller's chronological per-location
          sequential loop (inference.py's `run_autoregressive_inference`,
          self_pred_oof.py's `run_self_prediction_substituted_pass`).
        - self.prev_pred_lookup is not None (train/valid): a FIXED, precomputed realistic OOF
          prediction for the same (location, T-30min) row, from `load_prev_pred_lookup`. Same
          input-quality regime as production, by construction -- see that function's docstring.
        - Neither set (feature disabled, or lookup entry missing): zero value + mask=0, same
          convention as any other missing-observation slot.
        """
        ...
        if self.ar_cache is not None:
            ...
            if cached is None:
                return [zero_value, zero_mask]
            value = cached if torch.is_tensor(cached) else torch.from_numpy(np.asarray(cached, dtype=np.float32))
        ...
        if self.prev_pred_lookup is not None:
            cached = self.prev_pred_lookup.get(key)
            if cached is None:
                return [zero_value, zero_mask]
            value = torch.from_numpy(np.asarray(cached, dtype=np.float32)).reshape(1, *cached.shape)
            if value.shape[-2:] != self.target_size:
                value = _resize_chw(value, self.target_size)
            return [value, torch.ones((1, *self.target_size), dtype=torch.float32)]
