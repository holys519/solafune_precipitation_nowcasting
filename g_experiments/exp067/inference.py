#!/usr/bin/env python3
"""Run GPU inference for exp056 and write prediction GeoTIFFs.
...
def build_location_sequences(rows: list[dict[str, str]]) -> dict[str, list[dict[str, str]]]:
    ...
    features.autoregressive_prev_pred's chain depends on (each row's AR input is the previous
    row's own output, 30 minutes earlier, at the SAME location)."""
...
def run_autoregressive_inference(
    ...
    (ported from exp053's inference.py, same mechanics -- exp067's fix vs exp053 is entirely in
    dataset.py's training-time channel source, not this eval-time loop, which was never the
    problem: at real eval time there is no ground truth for T-30min either way).

    Sets `ds.ar_cache` to a fresh dict so the dataset sources the AR input channel ONLY from
    self-generated predictions. Processes rows in "steps": step i is the i-th row of every
    location's own chronological sequence, batched together across locations (independent) for
    GPU throughput; sequential only along each location's own time axis. The array cached for
    the next step is the SAME fully-postprocessed array written to disk for this step
    (calibration + rain_prob threshold + value threshold + non-negative clip already applied).
    """
...
def main() -> None:
    ...
    if features.get("autoregressive_prev_pred"):
        # Exposure-bias-affected path: true T-30min GPM is unavailable at eval time, so rows
        # are grouped by name_location, sorted chronologically, and processed step-by-step with
        # each row's own prediction threaded forward as the AR input for that location's next
        # row. See run_autoregressive_inference's docstring.
        print(
