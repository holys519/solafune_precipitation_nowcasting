#!/usr/bin/env python3
"""Train exp056: same strict-green input/CV pipeline as exp038, but with the new
FactorizedMeanShapeUNet architecture (mean-intensity x normalized-shape factorization) selectable
via config `model.architecture: factorized_mean_shape`. Unmodified from exp038 otherwise."""
...
def main() -> None:
    ...
    prev_pred_lookup = None
    if features.get("autoregressive_prev_pred"):
        prev_pred_lookup_path = resolve_path(config["data"]["prev_pred_lookup_npz"])
        # Built from ALL rows (not just this fold's train/valid split) so both arms can resolve
        # any (location, T-30min) key regardless of which fold that earlier row happens to fall
        # in -- see load_prev_pred_lookup's docstring for why this stays leakage-safe per fold.
    ...
    experiment_name = str(config.get("experiment", {}).get("name", "exp056"))
    print(
